...
exports.orderingRouter = orderingRouter;
...
function orderingRouter(io) {
    ...
    router.post("/checkout", async (req, res) => {
        ...
        const location = await prisma_1.prisma.location.findUnique({ where: { id: locationId } });
        if (!location)
        ...
        let dbCustomer = await prisma_1.prisma.customer.findFirst({ where: { email: customer.email } });
        if (!dbCustomer) {
            dbCustomer = await prisma_1.prisma.customer.create({
        ...
        try {
            ...
            const session = await stripe.checkout.sessions.create({
                ...
                line_items: lineItemsData.map((li) => ({
                    ...
                    price_data: {
                        ...
                        unit_amount: (0, stripe_1.toStripeAmount)(li.menuItem.price, location.currency),
            ...
            await prisma_1.prisma.order.update({ where: { id: order.id }, data: { stripeSessionId: session.id } });
